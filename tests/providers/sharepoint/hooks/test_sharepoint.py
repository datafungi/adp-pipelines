...
class FakeConnection:
    def __init__(self, login, password, extra_dejson):
        self.login = login
        self.password = password
        self.extra_dejson = extra_dejson


def _patch_get_connection(mocker, extra_dejson=None):
    conn = FakeConnection(
        login=CLIENT_ID,
...
@pytest.mark.parametrize(
    "login,password",
    [(None, CLIENT_SECRET), (CLIENT_ID, None), (None, None), ("", CLIENT_SECRET)],
)
def test_get_conn_missing_login_or_password_raises(mocker, login, password):
    conn = FakeConnection(
        login=login, password=password, extra_dejson={"tenant_id": TENANT_ID}
    )
    ...
    with pytest.raises(ValueError, match="login.*password|password.*login"):
        hook.get_conn()


def test_get_ui_field_behaviour():
    behaviour = SharePointHook.get_ui_field_behaviour()

    assert behaviour["hidden_fields"] == ["host", "schema", "port"]
    assert behaviour["relabeling"]["login"] == "Client ID"
    assert behaviour["relabeling"]["password"] == "Client Secret"
    assert "tenant_id" in behaviour["placeholders"]["extra"]
